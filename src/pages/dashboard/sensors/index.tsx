...
const Sensors: FC<Props> = () => {
  ...
  const handleEdit = (event: SyntheticEvent, row: Sensor) => {
    event.preventDefault();
    console.log("will edit", row);
  };

  const handleCopy = (event: SyntheticEvent, row: Sensor) => {
    event.preventDefault();
    setSelectedSensor(row);
  };

  const handleDelete = (event: SyntheticEvent, row: Sensor) => {
    event.preventDefault();
    console.log("will delete", row);
  };
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <IconButton onClick={(e) => handleCopy(e, params.row)}>
              <ContentCopyIcon />
            </IconButton>
  ...
  return (
    <Box sx={{ m: 1, p: 1 }}>
      ...
      >
        <MenuItem>
          {selectedSensor && (
            <EditSensor sensor={selectedSensor} />
          )}
        </MenuItem>
